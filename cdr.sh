...
curl -H 'content-type: application/json' "192.168.0.199/gui/cdr/cdr?__auth_user=****&__auth_pass=****d&sortby=end_timestamp&sortorder=desc&since=RANGE&rows=2000&between=${CurrentMonth}+${CurrentDay}%2C+${CurrentYear}&between=${CurrentMonth}+${CurrentDay}%2C+${CurrentYear}&show_outbound=0" > "/mnt/share/COMMON FILES/STAFF FOLDERS/Jeffrey Zic/CDRHourly/CDR/$NOW"
